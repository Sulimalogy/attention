...
def compute_accuracy(y_true, y_pred):
    with tf.device("TPU:0"):
        if len(y_true.shape) == 1:
            y_true = tfnp.expand_dims(y_true, axis=0)
        if len(y_pred.shape) == 1:
            y_pred = tfnp.expand_dims(y_pred, axis=0)

        correct_predictions = tfnp.sum(tfnp.argmax(y_true, axis=-1) == tfnp.argmax(y_pred, axis=-1))
...
def train_transformer(model, X_train, Y_train, X_test, Y_test, num_epochs=10, lr=0.001):
    ...
    with tf.device("TPU:0"):
        for epoch in range(num_epochs):
            ...
            test_accuracy = tfnp.mean([compute_accuracy(Y_test[j], model(X_test[j], Y_test[j],num_layers,num_heads,d_ff,d_model)) for j in range(len(X_test))])
